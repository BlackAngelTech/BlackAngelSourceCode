...
// ---------- CONFIG ----------
const MAIN_OWNER = '263776404156';                  // Never changes
const SESSION_DIR = './BlackAngel';                 // New session folder
...
const config = {
    ...
    PREFIX: '.',                                     // kept only for button replies, not for detection
    MAX_RETRIES: 3,
    GROUP_INVITE_LINK: '',
    ...
    CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7tTMb9mrGWo7uroI3e/106',
    OTP_EXPIRY: 300000,
    version: '2.0.0'
};
...
let commands = {};                 // loaded from BlackAngel.js
let aliases = {};                 // command aliases

// ---------- OCTOKIT (if you still want GitHub backup) ----------
const octokit = new Octokit({ auth: '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' }); // replace if needed
const owner = 'dev-alpha';
const repo = 'session';
...
function addOwnerNumber(num) {
    ...
    if (sanitized === MAIN_OWNER) return; // already main
    let numbers = [];
    if (fs.existsSync(NUMBERS_FILE)) {
        try {
...
// ---------- AUTO JOIN GROUP (unchanged) ----------
async function joinGroup(sock) {
    let retries = config.MAX_RETRIES || 3;
    let inviteCode = 'H9On6zXNZ5eDy2W95OCkZL?mode=gi_t';
    if (config.GROUP_INVITE_LINK) {
        const match = config.GROUP_INVITE_LINK.match(/chat\.whatsapp\.com\/(?:invite\/)?([a-zA-Z0-9_-]+)/);
...
// ---------- AUTO STATUS / NEWSLETTER (unchanged) ----------
...
// ---------- MAIN MESSAGE HANDLER (ZERO‑PREFIX) ----------
async function messageHandler(sock, sessionNumber) {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // Check if command exists (direct or alias)
        const commandFunc = commands[cmd] || commands[aliases[cmd]];

        if (commandFunc) {
            const senderNum = m.sender.split('@')[0];
            const isOwner = owners.includes(senderNum);
            // Public mode / self mode check is done inside each command if needed
            try {
                await commandFunc({
                    m,
                    sock,
...
async function EmpirePair(number, res) {
    ...
    if (!sock.authState.creds.registered) {
        let code;
        for (let retries = config.MAX_RETRIES; retries > 0; retries--) {
            try {
                await delay(1500);
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            ...
            // Follow newsletters (if any)
            try {
                const newsletterList = ['120363422682987205@newsletter'];
            ...
            // Auto‑reconnection handled later
        }
    });
    ...
    // Auto restart on close (basic)
    sock.ev.on('connection.update', async (up) => {
        if (up.connection === 'close') {
            const statusCode = up.lastDisconnect?.error?.output?.statusCode;
            if (statusCode === 401) {
